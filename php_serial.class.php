...
class phpSerial
{
   ...
   private $_winPortSettings = "baud=96 parity=n data=8 stop=1 octs=off rts=off xon=off dtr=off";
...
//			}
   ...
   public function setWindowsPortSettings($portSettings)
   {
      $this->_winPortSettings = $portSettings;
   }
   ...
   public function getOS()
   {
	...
	function deviceSet ($device)
	{
		if ($this->_dState !== SERIAL_DEVICE_OPENED)
		{
			...
			{
				if (preg_match("@^COM(\d+):?$@i", $device, $matches) and $this->_exec(exec("mode " . $device . " $this->_winPortSettings")) === 0)
				{
